...
func TestUpdateModelCatalogSettings_InvalidJSON400(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := &SettingHandler{} // 绑定失败在触达 service 之前返回
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPut, "/api/v1/admin/settings/model-catalog",
		strings.NewReader(`{"enabled": "not-a-bool"}`))
	c.Request.Header.Set("Content-Type", "application/json")

	h.UpdateModelCatalogSettings(c)
...
func TestUpdateModelCatalogSettings_MissingEnabled400(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := &SettingHandler{}
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
