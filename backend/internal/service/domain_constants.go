...
const (
	...
	// SettingKeyModelCatalogEnabled is a DB-backed soft switch for the fork's
	// "Model Catalog" view (model-centric pricing catalog, formerly named
	// "Model Plaza" before the upstream page of the same name was merged in).
	// When false: the user endpoint returns enabled=false with an empty list and
	// the sidebar entry is hidden. Defaults to false (opt-in feature).
	//
	// NOTE: 该 key 故意不进 SystemSettings/UpdateSettings 全量链路；
	// 专项端点 PUT /admin/settings/model-catalog 是唯一写入口。
	SettingKeyModelCatalogEnabled = "model_catalog_enabled"
