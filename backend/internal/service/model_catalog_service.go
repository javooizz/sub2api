...
// catalogGroupLister 提供全部 active 分组。GroupRepository 天然满足。
type catalogGroupLister interface {
	ListActive(ctx context.Context) ([]Group, error)
}

// catalogAccountLister 提供分组下账号（仓储已过滤 status=active，无时态谓词——
// "配置上可提供"取样口径，规格 §2）。AccountRepository 天然满足。
type catalogAccountLister interface {
	ListByGroup(ctx context.Context, groupID int64) ([]Account, error)
}

// catalogChannelLister 提供渠道原始数据（纯定价目录）。不用 ChannelService.ListAvailable——
// 它返回前会执行 fillGlobalPricingFallback，回落价会遮蔽其他渠道显式价（规格 §4.2 步骤 4）。
// ChannelRepository 天然满足。
type catalogChannelLister interface {
	ListAll(ctx context.Context) ([]Channel, error)
}

// catalogPricingCatalog 提供 LiteLLM 全局定价回落。*PricingService 天然满足。
type catalogPricingCatalog interface {
	GetModelPricing(model string) *LiteLLMModelPricing
}

// catalogGroupProvider 提供用户实际可绑定的分组（公开标准 + 授权专属 + 已订阅订阅型）。
// *APIKeyService 天然满足。
type catalogGroupProvider interface {
	GetAvailableGroups(ctx context.Context, userID int64) ([]Group, error)
}

// catalogConfigProvider 提供 model-plaza 的扩展配置。*ExtensionConfigService 天然满足。
type catalogConfigProvider interface {
	GetAdmin(ctx context.Context, agentID string) (*ExtensionConfigRecord, error)
}

// ===== 输出类型 =====

// CatalogGroupRef 广场视图中模型可用分组的引用。
...
type CatalogGroupRef struct {
	...
	ImagePricing *CatalogGroupImagePricing
}

// CatalogGroupImagePricing 分组出图计费展示信息（规格 2026-06-07 §4.2）。
// 展示优先级与真实计费 calculateOpenAIImageCost 一致：渠道按次价 > 分组图片价 > 平台默认价。
type CatalogGroupImagePricing struct {
...
// CatalogModel 广场视图中的一个模型条目。唯一身份 = (Platform, lower(Name))。
...
// ModelCatalogData 是 GetCatalogForUser 的返回值。
type ModelCatalogData struct {
	Announcement string
	Models       []CatalogModel
}

// ModelCatalogService 聚合"模型广场"视图（2026-06-05 修订，规格 §4.2/§9）：
// 模型↔分组关联以账号实际可用为准（运行时推导），渠道仅作纯定价目录。
// 可见性只作用于广场展示，不影响绑定/计费权限（后者由 GetAvailableGroups 把关）。
type ModelCatalogService struct {
	groups     catalogGroupLister
	accounts   catalogAccountLister
	channels   catalogChannelLister
	pricing    catalogPricingCatalog
	userGroups catalogGroupProvider
	config     catalogConfigProvider

	cacheMu     sync.RWMutex
	usableCache map[int64]catalogUsableCacheEntry
	now         func() time.Time // 测试注入；nil 时用 time.Now
}

// NewModelCatalogService 构造（wire 注入具体实现，接口在构造时收窄）。
...
func (s *ModelCatalogService) loadConfig(ctx context.Context) (domain.ModelPlazaExtensionConfig, error) {
...
// catalogAggKey 是模型聚合身份：(Platform, lowercase(Name))，跨平台同名模型互不串台。
type catalogAggKey struct {
	platform  string
	nameLower string
}

// catalogGroupModels 一个可见分组及其账号推导出的可用模型集。
// img* 为图像计费原始配置(从 Group 拷贝):ImagePricing 是 (模型×分组) 维度——
// fallback 档价依赖模型,故原始配置带到 aggregate 阶段 per-model 解析(规格 2026-06-07 §4.3)。
type catalogGroupModels struct {
	ref    CatalogGroupRef
	models []string
...
// catalogPriceEntry 显式价索引条目（displayName = 定价的原始大小写，模型身份的事实来源）。
type catalogPriceEntry struct {
	displayName string
	pricing     *ChannelModelPricing
}

// GetCatalogForUser 聚合当前用户可见的广场模型列表（规格 §4.2）。
//
// 可见分组（仅作用于广场展示）：
//
...
func (s *ModelCatalogService) GetCatalogForUser(ctx context.Context, userID int64) (*ModelCatalogData, error) {
	...
	visible := make([]catalogGroupModels, 0, len(allGroups))
	for i := range allGroups {
		...
		visible = append(visible, catalogGroupModels{
			ref: CatalogGroupRef{
	...
	return &ModelCatalogData{Announcement: cfg.Announcement, Models: s.aggregate(visible, priceIdx, &cfg)}, nil
}

// aggregate 倒排聚合 模型×分组，并完成定价 join、描述注入与排序（规格 §4.2 步骤 3–6）。
func (s *ModelCatalogService) aggregate(visible []catalogGroupModels, priceIdx map[catalogAggKey]catalogPriceEntry, cfg *domain.ModelPlazaExtensionConfig) []CatalogModel {
	agg := make(map[catalogAggKey]*CatalogModel)
	groupSeen := make(map[catalogAggKey]map[int64]struct{})
	for _, gm := range visible {
		for _, name := range gm.models {
			k := catalogAggKey{platform: gm.ref.Platform, nameLower: strings.ToLower(name)}
			entry, ok := agg[k]
			if !ok {
				entry = &CatalogModel{Name: name, Platform: gm.ref.Platform}
				agg[k] = entry
	...
	groupConf := make(map[int64]*catalogGroupModels, len(visible))
	for i := range visible {
		groupConf[visible[i].ref.ID] = &visible[i]
	}

	out := make([]CatalogModel, 0, len(agg))
	for k, m := range agg {
		...
		m.BillingMode = catalogDisplayBillingMode(m.Pricing)
		out = append(out, *m)
	}
...
func (s *ModelCatalogService) applyGroupImagePricing(m *CatalogModel, lp *LiteLLMModelPricing, groupConf map[int64]*catalogGroupModels, channelHit bool) {
	...
	var base *CatalogGroupImagePricing // 首个配价分组的解析档价(模型级基准)
	for i := range m.Groups {
		gc := groupConf[m.Groups[i].ID]
		if gc == nil {
			continue // 防御:Groups 均源自 visible,不可达
		}
		ip := &CatalogGroupImagePricing{Allowed: gc.imgAllow}
	...
	if base == nil {
		base = &CatalogGroupImagePricing{Price1K: &d1, Price2K: &d2, Price4K: &d4}
	}
...
func (s *ModelCatalogService) buildPricingIndex(ctx context.Context, excludedCh map[int64]struct{}) (map[catalogAggKey]catalogPriceEntry, error) {
	...
	idx := make(map[catalogAggKey]catalogPriceEntry)
	for i := range channels {
		...
		for _, m := range ch.SupportedModels() {
			...
			k := catalogAggKey{platform: m.Platform, nameLower: strings.ToLower(m.Name)}
			if _, dup := idx[k]; dup {
				continue
			}
			idx[k] = catalogPriceEntry{displayName: m.Name, pricing: m.Pricing}
		}
	}
	return idx, nil
}
...
// 显示名与 GetCatalogForUser 同口径（定价原始大小写优先，否则字典序最小写法），
// 保证管理员配置的复合键能在用户端字节精确命中。
func (s *ModelCatalogService) ListAllModelIdentities(ctx context.Context) ([]ModelIdentity, error) {
	allGroups, err := s.groups.ListActive(ctx)
	if err != nil {
		return nil, fmt.Errorf("list active groups: %w", err)
	}
	...
	display := make(map[catalogAggKey]string)
	for i := range allGroups {
		g := &allGroups[i]
		models, derr := s.groupUsableModels(ctx, g)
		if derr != nil {
		...
		for _, name := range models {
			k := catalogAggKey{platform: g.Platform, nameLower: strings.ToLower(name)}
...
// catalogDisplayBillingMode 归一展示计费模式：image/per_request 必须有每张价或带价档位
...
func catalogDisplayBillingMode(p *ChannelModelPricing) BillingMode {
