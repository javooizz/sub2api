...
func TestGetPlazaForUser_PublicStandardGroupVisible(t *testing.T) {
	...
	data, err := svc.GetCatalogForUser(context.Background(), 1)
...
func TestSynthesizeImageTierPricing(t *testing.T) {
	...
	// 合成物必须被 catalogDisplayBillingMode 识别为 image(带价档位)
	require.Equal(t, BillingModeImage, catalogDisplayBillingMode(p))
}
