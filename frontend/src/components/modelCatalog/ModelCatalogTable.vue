    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
      <thead>
        <tr class="text-left text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
          ...
          <th class="px-3 py-2">{{ t('modelCatalog.table.model') }}</th>
          <th class="px-3 py-2">{{ t('modelCatalog.table.provider') }}</th>
          <th class="hidden px-3 py-2 lg:table-cell">{{ t('modelCatalog.table.description') }}</th>
          <th class="px-3 py-2">{{ t('modelCatalog.table.billingType') }}</th>
          <th class="px-3 py-2">{{ t('modelCatalog.table.price') }}</th>
          <th class="hidden px-3 py-2 md:table-cell">{{ t('modelCatalog.table.groups') }}</th>
        </tr>
      </thead>
        ...
        >
          ...
          <td class="px-3 py-2.5">
            <span class="whitespace-nowrap text-xs text-gray-600 dark:text-gray-300">
              {{ t(`modelCatalog.billingMode.${m.billing_mode}`, m.billing_mode) }}
            </span>
          </td>
...
import { formatPerMillion, formatPerRequest, effectiveMultiplier, imageTierLines, isImageTierModel } from '@/utils/catalogPricing'
...
function activeGroupFor(m: CatalogModel): CatalogGroup | null {
...
function priceSummary(m: CatalogModel): string {
  const p = m.pricing
  if (!p) return t('modelCatalog.card.noPricing')
  ...
  if (isImageTierModel(m)) {
    ...
    if (tiers.length > 0) {
      return t('modelCatalog.table.imageFrom', { price: tiers[0].value })
    }
  }
  ...
  if (m.billing_mode === 'per_request' || m.billing_mode === 'image') {
    ...
    if (pr) return `${pr} ${t('modelCatalog.pricing.perCall')}`
  }
  ...
  return t('modelCatalog.card.noPricing')
}
</script>
