  ...
  <aside class="space-y-5">
    ...
    <section>
      <h4 class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
        {{ t('modelCatalog.filters.provider') }}
      </h4>
      <div class="space-y-1">
        <button
          type="button"
        ...
        >
          <span>{{ t('modelCatalog.filters.allProviders') }}</span>
    ...
    <section>
      ...
      <div class="space-y-1">
        ...
        >
          <span>{{ t('modelCatalog.filters.allGroups') }}</span>
        </button>
        <button
    ...
    <section>
      <h4 class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
        {{ t('modelCatalog.filters.billingType') }}
      </h4>
      <div class="space-y-1">
        <button
          type="button"
        ...
        >
          <span>{{ t('modelCatalog.filters.allBillingTypes') }}</span>
          <span class="filter-count">{{ totalCount }}</span>
        </button>
        <button
        ...
        >
          <span>{{ t(`modelCatalog.billingMode.${opt.value}`, opt.value) }}</span>
          <span class="filter-count">{{ opt.count }}</span>
        </button>
      </div>
    </section>
...
import { effectiveMultiplier } from '@/utils/catalogPricing'
import type { CatalogGroup } from '@/api/modelCatalog'
import type { PlazaFilterState, FilterOption } from '@/composables/useModelCatalogFilters'

const props = defineProps<{
  filters: PlazaFilterState
  platformOptions: FilterOption[]
  billingModeOptions: FilterOption[]
  groupOptions: CatalogGroup[]
  totalCount: number
...
function multiplierLabel(grp: CatalogGroup): string {
