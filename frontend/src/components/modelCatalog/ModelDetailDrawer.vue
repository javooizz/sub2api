    ...
    <Transition name="drawer-slide">
      ...
      >
        ...
        <header class="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-gray-700">
          <div class="flex min-w-0 items-center gap-3">
            ...
            <button
              ...
              :title="t('modelCatalog.card.copyModelName')"
              :aria-label="t('modelCatalog.card.copyModelName')"
        ...
        <div class="flex-1 space-y-6 overflow-y-auto px-5 py-4">
          <!-- 基本信息 -->
          <section>
            <h3 class="detail-section-title">{{ t('modelCatalog.detail.basicInfo') }}</h3>
            <p class="text-sm text-gray-600 dark:text-gray-300">
              {{ model.description || t('modelCatalog.detail.noDescription') }}
            </p>
            <span
            ...
            >
              {{ t(`modelCatalog.billingMode.${model.billing_mode}`, model.billing_mode) }}
            </span>
          </section>

          <!-- 标准价格 -->
          <section>
            <h3 class="detail-section-title">{{ t('modelCatalog.detail.basePricing') }}</h3>
            ...
            <p v-else class="text-sm text-gray-400">{{ t('modelCatalog.card.noPricing') }}</p>
              ...
              <table class="w-full text-xs">
                <thead>
                  <tr class="text-left text-gray-500 dark:text-gray-400">
                    <th class="py-1 pr-2 font-medium">{{ t('modelCatalog.detail.tierRange') }}</th>
                    <th class="py-1 pr-2 font-medium">{{ t('modelCatalog.pricing.input') }}</th>
                    <th class="py-1 font-medium">{{ t('modelCatalog.pricing.output') }}</th>
                  </tr>
                </thead>
          ...
          <!-- 分组价格对比 -->
          <section>
            <h3 class="detail-section-title">{{ t('modelCatalog.detail.groupPricing') }}</h3>
            <p class="mb-2 text-xs text-gray-400 dark:text-gray-500">
              {{ t('modelCatalog.detail.groupPricingHint') }}
            </p>
            <div class="space-y-2">
              <div
              ...
              >
                <div class="mb-1.5 flex items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-1.5">
                    ...
                    >
                      ...
                      {{ t('modelCatalog.filters.requiresSubscription') }}
                    </span>
                    <span
                    ...
                    >
                      {{ t('modelCatalog.card.userRateBadge') }}
                    </span>
                  </div>
                  <span
                ...
                <p
                  ...
                  role="note"
                >
                  {{ t('modelCatalog.detail.noImageGeneration') }}
                </p>
                ...
                >
                  {{ t('modelCatalog.detail.goSubscribe') }}
...
import type { CatalogModel, CatalogGroup, CatalogPricingInterval } from '@/api/modelCatalog'
...
function buildPriceRows(m: CatalogModel, mult: number): PriceRow[] {
  const p = m.pricing
  if (!p) return []
  const perM = t('modelCatalog.pricing.perMillionTokens')
  const perCall = t('modelCatalog.pricing.perCall')
  const rows: PriceRow[] = []
  ...
  if (m.billing_mode === 'per_request' || m.billing_mode === 'image') {
    push(t('modelCatalog.pricing.perRequest'), formatPerRequest(p.per_request_price, mult), perCall)
  }
  push(t('modelCatalog.pricing.input'), formatPerMillion(p.input_price, mult), perM)
  push(t('modelCatalog.pricing.output'), formatPerMillion(p.output_price, mult), perM)
  // 图像输出价不限模式：gpt-image 系是 token 模式但按图像 token 计费（与 ModelCard 同理）
  push(t('modelCatalog.pricing.imageOutput'), formatPerMillion(p.image_output_price, mult), perM)
  push(t('modelCatalog.pricing.cacheRead'), formatPerMillion(p.cache_read_price, mult), perM)
  push(t('modelCatalog.pricing.cacheWrite'), formatPerMillion(p.cache_write_price, mult), perM)
  return rows
}

/** 图像三档 → PriceRow 行（g=null 为基准）。 */
function imageRows(g: CatalogGroup | null): PriceRow[] {
  if (!props.model) return []
  return imageTierLines(props.model, g, props.userRates).map((tl) => ({
    label: t('modelCatalog.pricing.imageTier', { tier: tl.tier }),
    value: tl.value,
    unit: t('modelCatalog.pricing.perImage'),
  }))
}
...
function groupPriceRows(g: CatalogGroup): PriceRow[] {
  if (!props.model) return []
  if (isImage.value) {
...
function groupBadgeMultiplier(g: CatalogGroup): number {
  return isImage.value ? effectiveImageMultiplier(g, props.userRates) : effectiveMultiplier(g, props.userRates)
}

function intervalRange(iv: CatalogPricingInterval): string {
