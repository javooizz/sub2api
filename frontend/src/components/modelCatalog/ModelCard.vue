<template>
  ...
  >
    ...
    <div class="space-y-1 text-xs">
      ...
      <p v-else class="text-gray-400 dark:text-gray-500">{{ t('modelCatalog.card.noPricing') }}</p>
    </div>
    ...
    <div class="mt-auto flex flex-wrap items-center gap-1.5 pt-1">
      ...
      >
        {{ t('modelCatalog.card.dynamicTiers', { count: model.pricing!.intervals.length }) }}
      </span>
      <span
      ...
      >
        {{ t('modelCatalog.card.multiplierBadge', { multiplier: activeMultiplier }) }}
      </span>
      <span class="ml-auto text-[11px] text-gray-400">
        {{ t('modelCatalog.card.groupCount', { count: model.groups.length }) }}
      </span>
    </div>
  </article>
</template>
...
import type { CatalogModel, CatalogGroup } from '@/api/modelCatalog'
...
const emit = defineEmits<{
  open: [model: CatalogModel]
  copy: [name: string]
  'toggle-select': [model: CatalogModel]
}>()

const { t } = useI18n()

/** 选中分组（且模型挂在该分组）→ 该分组；否则 null（基准价）。 */
const activeGroup = computed<CatalogGroup | null>(() => {
...
const multiplierTitle = computed(() => {
  ...
  return props.userRates[g.id] !== undefined
    ...
    : t('modelCatalog.card.groupPrice', { group: g.name })
})
...
const priceLines = computed<PriceLine[]>(() => {
  ...
  if (props.model.billing_mode === 'per_request' || props.model.billing_mode === 'image') {
    ...
    if (pr) lines.push({ label: t('modelCatalog.pricing.perRequest'), value: pr, unit: perCall })
  }
  const input = formatPerMillion(p.input_price, m)
  if (input) lines.push({ label: t('modelCatalog.pricing.input'), value: input, unit: perM })
  const output = formatPerMillion(p.output_price, m)
  if (output) lines.push({ label: t('modelCatalog.pricing.output'), value: output, unit: perM })
  ...
  if (img) lines.push({ label: t('modelCatalog.pricing.imageOutput'), value: img, unit: perM })
  const cacheRead = formatPerMillion(p.cache_read_price, m)
  if (cacheRead) lines.push({ label: t('modelCatalog.pricing.cacheRead'), value: cacheRead, unit: perM })
  const cacheWrite = formatPerMillion(p.cache_write_price, m)
  if (cacheWrite) lines.push({ label: t('modelCatalog.pricing.cacheWrite'), value: cacheWrite, unit: perM })
  ...
  if (lines.length === 0 && p.intervals.length > 0) {
    ...
    if (fin) lines.push({ label: t('modelCatalog.pricing.input'), value: fin, unit: perM })
    const fout = formatPerMillion(first.output_price, m)
    if (fout) lines.push({ label: t('modelCatalog.pricing.output'), value: fout, unit: perM })
  }
