      ...
      <div class="min-w-0 flex-1 space-y-4">
        ...
        >
          <h3 class="mb-1 text-sm font-semibold text-gray-900 dark:text-gray-100">
            {{ t('modelCatalog.announcementTitle') }}
          </h3>
        ...
        <div v-else-if="loadError" class="card flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('modelCatalog.loadFailed') }}</p>
        ...
        <div v-else-if="!enabled" class="card flex flex-col items-center gap-2 py-16 text-center">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('modelCatalog.disabled') }}</p>
        </div>
        ...
        <div v-else-if="filtered.length === 0" class="card flex flex-col items-center gap-2 py-16 text-center">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ models.length === 0 ? t('modelCatalog.empty') : t('modelCatalog.emptyFiltered') }}
          </p>
        </div>
...
import ModelCatalogFilters from '@/components/modelCatalog/ModelCatalogFilters.vue'
import ModelCatalogToolbar from '@/components/modelCatalog/ModelCatalogToolbar.vue'
import ModelCard from '@/components/modelCatalog/ModelCard.vue'
import ModelCatalogTable from '@/components/modelCatalog/ModelCatalogTable.vue'
import ModelDetailDrawer from '@/components/modelCatalog/ModelDetailDrawer.vue'
import modelCatalogAPI, { type CatalogModel } from '@/api/modelCatalog'
...
async function load() {
  ...
  try {
    ...
    const [plaza, rates] = await Promise.all([
      modelCatalogAPI.getModelCatalog(),
    ...
    appStore.showError(extractApiErrorMessage(err, t('modelCatalog.loadFailed')))
  } finally {
    loading.value = false
  }
}
...
async function copyName(name: string) {
  try {
    ...
    appStore.showSuccess(t('modelCatalog.card.copied'))
  } catch {
