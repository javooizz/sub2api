    ...
    <div class="mx-auto max-w-4xl space-y-5 pb-24">
      ...
      <section class="card p-5">
        <h2 class="mb-1 text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ t('admin.extensionConfig.modelCatalog.enableSection') }}
        </h2>
        <p class="mb-4 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.extensionConfig.modelCatalog.enableHint') }}
        </p>
        <label class="flex w-fit cursor-pointer items-center gap-3">
          ...
          <span class="text-sm font-medium text-gray-900 dark:text-gray-100">
            {{ t('admin.extensionConfig.modelCatalog.enableLabel') }}
          </span>
        </label>
      </section>
      ...
      <section class="card p-5">
        <h2 class="mb-4 text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ t('admin.extensionConfig.modelCatalog.exclusionSection') }}
        </h2>
        <div class="grid gap-6 md:grid-cols-2">
          <div>
            <h3 class="mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">
              {{ t('admin.extensionConfig.modelCatalog.excludedChannels') }}
            </h3>
            <p class="mb-2 text-xs text-gray-400">
              {{ t('admin.extensionConfig.modelCatalog.excludedChannelsHint') }}
            </p>
            <div class="max-h-56 space-y-1 overflow-y-auto rounded-lg border border-gray-200 p-2 dark:border-gray-700">
              <p v-if="channels.length === 0" class="p-2 text-xs text-gray-400">
                {{ t('admin.extensionConfig.modelCatalog.noChannels') }}
              </p>
              <label
          ...
          <div>
            <h3 class="mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">
              {{ t('admin.extensionConfig.modelCatalog.excludedGroups') }}
            </h3>
            <p class="mb-2 text-xs text-gray-400">
              {{ t('admin.extensionConfig.modelCatalog.excludedGroupsHint') }}
            </p>
            <div class="max-h-56 space-y-1 overflow-y-auto rounded-lg border border-gray-200 p-2 dark:border-gray-700">
              <p v-if="groups.length === 0" class="p-2 text-xs text-gray-400">
                {{ t('admin.extensionConfig.modelCatalog.noGroups') }}
              </p>
              <label
                v-for="g in groups"
                :key="g.id"
      ...
      <section class="card p-5">
        <h2 class="mb-1 text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ t('admin.extensionConfig.modelCatalog.descriptionsSection') }}
        </h2>
        <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.extensionConfig.modelCatalog.descriptionsHint') }}
        </p>
        <input
          v-model="modelSearch"
          type="text"
          class="input mb-3 w-full sm:w-64"
          :aria-label="t('admin.extensionConfig.modelCatalog.searchModels')"
          :placeholder="t('admin.extensionConfig.modelCatalog.searchModels')"
        />
        <div class="max-h-96 space-y-2 overflow-y-auto">
          ...
          >
            ...
            <input
              ...
              :aria-label="m.name"
              :placeholder="t('admin.extensionConfig.modelCatalog.descriptionPlaceholder')"
        ...
        <div v-if="orphanDescriptions.length > 0" class="mt-3 rounded-lg bg-amber-50 p-3 text-xs text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
          <p class="mb-1 font-medium">{{ t('admin.extensionConfig.modelCatalog.orphanDescriptions') }}</p>
      ...
      <section class="card p-5">
        <h2 class="mb-1 text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ t('admin.extensionConfig.modelCatalog.announcementSection') }}
        </h2>
        <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.extensionConfig.modelCatalog.announcementHint') }}
        </p>
        <textarea
          ...
          :aria-label="t('admin.extensionConfig.modelCatalog.announcementSection')"
          :placeholder="t('admin.extensionConfig.modelCatalog.announcementPlaceholder')"
        />
      ...
      >
        <div class="mx-auto flex max-w-4xl items-center justify-end gap-3">
          <button type="button" class="btn btn-secondary" :disabled="saving" @click="discard">
            {{ t('admin.extensionConfig.modelCatalog.discard') }}
          </button>
          <button type="button" class="btn btn-primary" :disabled="saving" @click="save">
            {{ saving ? t('admin.extensionConfig.modelCatalog.saving') : t('admin.extensionConfig.modelCatalog.save') }}
          </button>
        </div>
      </div>
    </div>
...
/**
 * 扩展配置 → 模型广场
 * 开关存 settings 专项端点（PUT /admin/settings/model-catalog，不进全量 settings）；
...
import { extensionConfigAPI, type ModelCatalogConfig } from '@/api/admin/extensionConfig'
import adminModelCatalogAPI, { type ModelIdentity } from '@/api/admin/modelCatalog'
...
const form = ref<ModelCatalogConfig>({
...
function discard() {
  ...
  if (!window.confirm(t('admin.extensionConfig.modelCatalog.discardConfirm'))) return
  loadAll().then(() => appStore.showInfo(t('admin.extensionConfig.modelCatalog.discarded')))
}
...
async function loadAll() {
  ...
  try {
    const [settings, channelPage, allGroups, models, cfg] = await Promise.all([
      adminModelCatalogAPI.getSettings(),
      listChannels(1, 200),
      groupsAPI.getAll(),
      adminModelCatalogAPI.listModels(),
    ...
    appStore.showError(extractApiErrorMessage(e, t('admin.extensionConfig.modelCatalog.loadFailed')))
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    await adminModelCatalogAPI.updateSettings({ enabled: enabled.value })
  } catch {
    failures.push(t('admin.extensionConfig.modelCatalog.switchSaveFailed'))
  }
  try {
    ...
    failures.push(extractApiErrorMessage(e, t('admin.extensionConfig.modelCatalog.configSaveFailed')))
  }
  saving.value = false
  ...
  baselineSnapshot.value = snapshot()
  appStore.showSuccess(t('admin.extensionConfig.modelCatalog.saved'))
}

// 离开守卫（同工作台页模式）
onBeforeRouteLeave(() => {
  if (!isDirty.value) return true
  return window.confirm(t('admin.extensionConfig.modelCatalog.discardConfirm'))
})
