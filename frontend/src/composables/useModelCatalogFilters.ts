...
import type { CatalogGroup, CatalogModel } from '@/api/modelCatalog'
...
function matchesSearch(m: CatalogModel, q: string): boolean {
  return m.name.toLowerCase().includes(q) || (m.description || '').toLowerCase().includes(q)
}

export function useModelCatalogFilters(models: Ref<CatalogModel[]>) {
  ...
  /** 应用全部筛选后的模型列表。 */
  const filtered = computed<CatalogModel[]>(() => {
  ...
  const searchScoped = computed<CatalogModel[]>(() => {
    const q = filters.search.trim().toLowerCase()
    if (!q) return models.value
    return models.value.filter((m) => matchesSearch(m, q))
  })

  function countBy(key: (m: CatalogModel) => string): FilterOption[] {
   ...
   * （后端按用户统一计算），故取首个对象即可。
   */
  const groupOptions = computed<CatalogGroup[]>(() => {
    const seen = new Map<number, CatalogGroup>()
