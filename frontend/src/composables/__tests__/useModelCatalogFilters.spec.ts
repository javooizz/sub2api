...
function g(id: number, overrides: Partial<CatalogGroup> = {}): CatalogGroup {
  return {
    id,
    name: `g${id}`,
    platform: 'anthropic',
...
describe('useModelCatalogFilters', () => {
  it('无筛选时返回全部', () => {
    const { filtered } = useModelCatalogFilters(ref(dataset))
    expect(filtered.value).toHaveLength(4)
  })

  it('按平台筛选', () => {
    const { filters, filtered } = useModelCatalogFilters(ref(dataset))
  ...
  it('platformOptions 按 search 过滤后计数并按名排序', () => {
    const { filters, platformOptions } = useModelCatalogFilters(ref(dataset))
  ...
  it('groupOptions 全量去重并按名排序', () => {
    const { groupOptions } = useModelCatalogFilters(ref(dataset))
    expect(groupOptions.value.map((x) => x.id)).toEqual([1, 2, 3])
  })

  it('selectedGroup 返回选中的分组对象', () => {
    const { filters, selectedGroup } = useModelCatalogFilters(ref(dataset))
  ...
  it('reset 清空全部筛选', () => {
    const { filters, filtered, reset } = useModelCatalogFilters(ref(dataset))
    filters.platform = 'openai'
    filters.groupId = 3
    filters.billingMode = 'token'
    filters.search = 'gpt'
